...
func validateRule(file, service, method string, rule *authnpb.AuthnRule) error {
	...
	case authnpb.AuthnMode_AUTHN_MODE_UNSPECIFIED:
		if len(rule.GetSchemes()) > 0 {
			return fmt.Errorf(
				...
				file, service, method, rule.GetSchemes(),
			)
		}
	...
	default:
		return fmt.Errorf("%s: service %s method %s: unknown AuthnMode %d", file, service, method, show)
	}
	return nil
}

func writeFile(g *protogen.GeneratedFile, packageName protogen.GoPackageName, rules []ruleplan.Entry[*authnpb.AuthnRule]) {
	authnPackage := protogen.GoImportPath("github.com/Servora-Kit/servora-platform/api/gen/go/platform/authn/v1")
