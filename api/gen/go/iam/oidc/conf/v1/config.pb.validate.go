...
func (m *OIDC) validate(all bool) error {
	...
	if all {
		switch v := interface{}(m.GetServiceAccessTokenTtl()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
				errors = append(errors, OIDCValidationError{
					field:  "ServiceAccessTokenTtl",
					reason: "embedded message failed validation",
					cause:  err,
				})
			}
	...
	} else if v, ok := interface{}(m.GetServiceAccessTokenTtl()).(interface{ Validate() error }); ok {
		if err := v.Validate(); err != nil {
			return OIDCValidationError{
				field:  "ServiceAccessTokenTtl",
				reason: "embedded message failed validation",
