...
func (tx *Tx) init() {
	...
	tx.OAuthTokenSession = NewOAuthTokenSessionClient(tx.config)
	tx.OIDCAuthorizationRequest = NewOIDCAuthorizationRequestClient(tx.config)
	tx.OIDCSigningKey = NewOIDCSigningKeyClient(tx.config)
	tx.PasswordAuthenticator = NewPasswordAuthenticatorClient(tx.config)
