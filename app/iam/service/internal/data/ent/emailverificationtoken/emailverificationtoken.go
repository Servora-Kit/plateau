// Code generated by ent, DO NOT EDIT.

package emailverificationtoken

import (
	"time"

	"entgo.io/ent/dialect/sql"
)

const (
	// Label holds the string label denoting the emailverificationtoken type in the database.
	Label = "email_verification_token"
	// FieldID holds the string denoting the id field in the database.
	FieldID = "id"
	// FieldUserID holds the string denoting the user_id field in the database.
	FieldUserID = "user_id"
	// FieldLoginIdentifierID holds the string denoting the login_identifier_id field in the database.
	FieldLoginIdentifierID = "login_identifier_id"
	// FieldTokenHash holds the string denoting the token_hash field in the database.
	FieldTokenHash = "token_hash"
	// FieldExpiresTime holds the string denoting the expires_time field in the database.
	FieldExpiresTime = "expires_time"
	// FieldConsumedTime holds the string denoting the consumed_time field in the database.
	FieldConsumedTime = "consumed_time"
	// FieldCreateTime holds the string denoting the create_time field in the database.
	FieldCreateTime = "create_time"
	// Table holds the table name of the emailverificationtoken in the database.
	Table = "email_verification_tokens"
)

// Columns holds all SQL columns for emailverificationtoken fields.
var Columns = []string{
	FieldID,
	FieldUserID,
	FieldLoginIdentifierID,
	FieldTokenHash,
	FieldExpiresTime,
	FieldConsumedTime,
	FieldCreateTime,
}

// ValidColumn reports if the column name is valid (part of the table columns).
func ValidColumn(column string) bool {
	for i := range Columns {
		if column == Columns[i] {
			return true
		}
	}
	return false
}

var (
	// UserIDValidator is a validator for the "user_id" field. It is called by the builders before save.
	UserIDValidator func(string) error
	// LoginIdentifierIDValidator is a validator for the "login_identifier_id" field. It is called by the builders before save.
	LoginIdentifierIDValidator func(string) error
	// TokenHashValidator is a validator for the "token_hash" field. It is called by the builders before save.
	TokenHashValidator func(string) error
	// DefaultCreateTime holds the default value on creation for the "create_time" field.
	DefaultCreateTime func() time.Time
)

// OrderOption defines the ordering options for the EmailVerificationToken queries.
type OrderOption func(*sql.Selector)

// ByID orders the results by the id field.
func ByID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldID, opts...).ToFunc()
}

// ByUserID orders the results by the user_id field.
func ByUserID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldUserID, opts...).ToFunc()
}

// ByLoginIdentifierID orders the results by the login_identifier_id field.
func ByLoginIdentifierID(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldLoginIdentifierID, opts...).ToFunc()
}

// ByTokenHash orders the results by the token_hash field.
func ByTokenHash(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldTokenHash, opts...).ToFunc()
}

// ByExpiresTime orders the results by the expires_time field.
func ByExpiresTime(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldExpiresTime, opts...).ToFunc()
}

// ByConsumedTime orders the results by the consumed_time field.
func ByConsumedTime(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldConsumedTime, opts...).ToFunc()
}

// ByCreateTime orders the results by the create_time field.
func ByCreateTime(opts ...sql.OrderTermOption) OrderOption {
	return sql.OrderByField(FieldCreateTime, opts...).ToFunc()
}
