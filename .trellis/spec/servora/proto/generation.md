...
`api/gen/go` 是 Servora 根 Go module 内的生成 package，只由 `just gen`/Buf 维护，禁止手改。日常 `just gen` 不 clean；删除/重命名 Proto 或移除 plugin 时，`just gen-fresh` 才显式清理后重建，规则来自 [`api/AGENTS.md`](../../../../../servora/api/AGENTS.md)。
