...
- [servora/framework/crud.md](../../../../.trellis/spec/servora/framework/crud.md) 已覆盖 `ResourcePlan`、`ListPreparer`、`ResourceNameMatcher`、`ListQuery`、mask/lifecycle、mapper、Ent `ListFields`/`ClearHelper`、scope fingerprint 与框架错误；[service/backend/crud.md](../../../../.trellis/spec/service/backend/crud.md) 已覆盖消费方构造和 `ToResponse`、`Columns`、`Bind`、`ToDTO` 等检查清单。后续只补具体历史仍有效但正文没有的边界，不能再笼统建议“补 CRUD 基础合同”。
- [servora/proto/annotations.md](../../../../.trellis/spec/servora/proto/annotations.md) 已明确 `SectionKey`、`SectionOptional`、`ApplyDefaults`、`CheckRequired`、`ApplyConf` 及 default 子树的 allocate 规则；历史 `ValidateConf` 名称不应回写。`bootstrap.md` 已说明 scan 顺序、optional 缺失和错误停止。
- [servora/web/proto-utils.md](../../../../.trellis/spec/servora/web/proto-utils.md) 已明确当前 TypeScript `moduleResolution: bundler`、ESM `.js` type-only import、ProtoJSON 64 位整数和验证入口；历史 Node16/NodeNext 是兼容目标或旧约束，不能当作当前配置。
- [plateau/security/capabilities.md](../../../../.trellis/spec/plateau/security/capabilities.md) 已覆盖 v2 前缀、签名 challenge、Redis Lua/GETDEL 一次性消费、TTL、scope、未知字段拒绝、失败关闭和多实例边界；当前明确不读取旧有状态 token，旧 token 兼容 Scenario 不应作为现行规则补写。
...
- [service/backend/data.md](../../../../.trellis/spec/service/backend/data.md) 已覆盖 schema owner、软删除、scope fingerprint、事务责任、错误保留和真实数据库验证边界；[api/proto/generation.md](../../../../.trellis/spec/api/proto/generation.md) 已覆盖根/leaf 生成顺序、输出 owner、TS sidecar、插件来源和验证命令；[web/client/http.md](../../../../.trellis/spec/web/client/http.md) 已覆盖同源、JSON/ProtoJSON 分流、取消/超时/GET 重试和写请求不重试。
