...
- [ ] AC7（R6）：两仓生成物、示例和实际入口一致，记录 Go/schema/插件来源。本地临时覆盖验证与真实版本独立消费分开记录；后者须使用可解析的新版本与对应 BSR schema，缺发布授权时明确阻塞，不冒充整个任务完成。
...
本地实施及验证已完成，结果见 research/current-state.md 第 11 节。任务保持 in_progress：AC7 的正式新版本独立消费验证尚未完成，提交、发布、BSR 推送及部署没有授权；未将本地覆盖验证冒充整个任务完成。
