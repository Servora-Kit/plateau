...
func Server[T jwtlib.Claims](authenticator *Authenticator, newClaims func() T, mapActor func(T) (security.Actor, error), opts ...authnruntime.Option) middleware.Middleware {
	...
	return func(handler middleware.Handler) middleware.Handler {
		return func(ctx context.Context, request any) (any, error) {
			...
			default:
				return nil, apiError(fmt.Errorf("jwt authn: unsupported mode %s", rule.GetMode()))
			}
...
func apiError(err error) error {
	...
	switch {
	case errors.Is(err, ErrMissingCredentials), errors.Is(err, ErrMalformedCredentials), errors.Is(err, ErrInvalidToken), errors.Is(err, ErrActorMapping):
